...
export function ExerciseSketch({ prompt, sketchUrl, className }: ExerciseSketchProps) {
    ...
    return (
        <div className={cn(
            ...
            {sketchUrl && !imageError ? (
                <div className="relative w-full h-full flex items-center justify-center p-4">
                    <img
                        src={sketchUrl}
                        alt={prompt}
                        className="max-w-full max-h-full object-contain filter invert brightness-200 contrast-150 group-hover:scale-110 transition-transform duration-700"
                        onError={() => setImageError(true)}
                    />
                    <div className="absolute top-4 left-4 flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">Live Sketch Asset</span>
                    </div>
                </div>
            ) : (
                <div className="relative z-10 flex flex-col items-center text-center p-6">
                    <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-400 mb-4 group-hover:scale-110 transition-transform duration-500 border border-emerald-500/20">
